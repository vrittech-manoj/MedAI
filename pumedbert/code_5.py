...
class AdvancedPubMedBERTClassifier(nn.Module):
    def __init__(self, n_classes, dropout_rate=0.2, hidden_size=768, model_version="v1"):
        super(AdvancedPubMedBERTClassifier, self).__init__()
        
        # Store model configuration for consistency
        ...
        # Consistent architecture - always use the same structure
        self.dropout1 = nn.Dropout(dropout_rate)
        self.classifier = nn.Linear(hidden_size, n_classes)
        
        # Freeze BERT layers initially for small datasets
        self._freeze_bert_layers()
        
    def _freeze_bert_layers(self):
        """Freeze BERT layers to prevent overfitting on small datasets"""
...
class ScalableMedicalDiseaseClassifier:
    def __init__(self, model_name='microsoft/BiomedNLP-PubMedBERT-base-uncased-abstract-fulltext'):
        ...
        self.model_config = None  # Store model configuration
    ...
    def train_model(self, csv_path, epochs=8, batch_size=8, learning_rate=2e-5, 
        ...
        # Initialize model with consistent configuration
        n_classes = len(self.label_encoder.classes_)
        self.model_config = {
            'n_classes': n_classes,
            'dropout_rate': 0.3,
            'hidden_size': 768,  # Always use 768 for BERT base
            'model_version': 'v1'
        }
        
        self.model = AdvancedPubMedBERTClassifier(**self.model_config)
    ...
    def save_checkpoint(self, epoch, val_f1, filename):
        """Save model checkpoint with consistent configuration"""
        checkpoint = {
            'model_state_dict': self.model.state_dict(),
            'model_config': self.model_config,  # Save model configuration
    ...
    def load_model(self, model_path):
        """Load trained model with architecture consistency checks"""
        ...
        if 'model_config' in checkpoint:
            ...
            logger.info(f"Using saved model config: {self.model_config}")
        else:
            ...
            logger.warning("No model config found in checkpoint, using default config")
        ...
        try:
            ...
            logger.info("Attempting to fix architecture mismatch...")
            ...
            for key, value in state_dict.items():
                if key in model_state_dict:
                    if value.shape == model_state_dict[key].shape:
                        ...
                        logger.warning(f"Shape mismatch for {key}: saved {value.shape}, expected {model_state_dict[key].shape}")
                else:
                    logger.warning(f"Unexpected key in checkpoint: {key}")
            
            # Load the filtered state dict
            self.model.load_state_dict(filtered_state_dict, strict=False)
            logger.info("Model loaded with filtered state dict")
...
def clean_start_training(csv_path, **kwargs):
    """Clean start training - removes old model files first"""
    import glob
    
    # Remove old model files
    old_files = glob.glob('best_medical_model.pt') + glob.glob('training_history.json')
    ...
    return train_production_model(csv_path, **kwargs)
...
if __name__ == "__main__":
    ...
    # Test symptoms
    test_symptoms = [
        "High fever,Joint pain,Skin rash,Headache,Muscle pain",
        "Cough,Chest pain,Sleep trouble,Wheeze,Rapid breathing",
        "Cough,Fever,Chills,Chest pain,Sweating",
    ]
