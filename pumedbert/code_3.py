...
class ScalableMedicalDiseaseClassifier:
    ...
    #     """Create data loaders with proper stratification"""
    ...
    def create_data_loaders(self, df, batch_size=8, num_workers=0):
        """Create data loaders using the full dataset for training"""
        logger.info(f"Creating data loaders with batch_size={batch_size}, num_workers={num_workers}")
        ...
        # Return only the training loader, since we are not using a test/validation set
        return train_loader, None, None, train_df
